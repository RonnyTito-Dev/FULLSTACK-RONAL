...
const PaymentMethodForm = () => {
  ...
  // Fetch payment method data if editing
  useEffect(() => {
    if (id) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        ...
        await createLog({ action: 'Editó un metodo de pago', affected_table: 'payment_methods'});
        Swal.fire({
          icon: 'success',
          title: '¡Actualizado!',
        ...
        await createLog({ action: 'Agregó un metodo de pago', affected_table: 'payment_methods'});
        Swal.fire({
          icon: 'success',
          title: '¡Creado!',
          text: 'El método de pago ha sido creado correctamente',
          confirmButtonColor: '#3085d6'
        });
      }
      navigate('/metodos-pago');
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.response?.data?.message || 'Ocurrió un error al guardar el método de pago',
        confirmButtonColor: '#3085d6'
      });
    } finally {
      setLoading(false);
    ...
    <div className="payment-method-form">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="mb-0 d-flex align-items-center">
          ...
          {id ? 'Editar Método de Pago' : 'Nuevo Método de Pago'}
        </h2>
        <button 
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    {id ? 'Actualizar Método' : 'Guardar Método'}
                  </>
                )}
              </button>
            </div>
